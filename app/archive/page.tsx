...
export default function ArchivePage() {
  ...
  const [date, setDate] = useState<string>(() => {
    if (typeof window === "undefined") return "";
    return sessionStorage.getItem("archive-date") || "";
  });
  const [page, setPage] = useState<number>(() => {
    if (typeof window === "undefined") return 1;
    const p = sessionStorage.getItem("archive-page");
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("archive-category", category);
    sessionStorage.setItem("archive-format", format);
    sessionStorage.setItem("archive-date", date);
    sessionStorage.setItem("archive-page", String(page));
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (isRestored) {
        sessionStorage.setItem("archive-scroll-pos", window.scrollY.toString());
      }
    };
  ...
  useEffect(() => {
    if (isRestored && !isLoading && dataMemo && dataMemo.length > 0) {
      const savedScroll = sessionStorage.getItem("archive-scroll-pos");
      if (savedScroll) {
        setTimeout(() => {
