...
export default function SeriesDetailPage({
  ...
  const seasonId =
    selectedSeasonId || leagueData?.currentseason?.id || getLatestSeasonId();

  const { data: fixturesData, isLoading: fixturesLoading } = useSWR(
    activeTab === "matches" && params.id
      ? `/api/leagues/${params.id}/fixtures`
      : null,
    fetcher,
  ...
  useEffect(() => {
    if (activeTab === "matches" && sortedFixtures.length > 0) {
      ...
      const todayHasMatches = sortedFixtures.some(
        (m: any) => m.starting_at?.slice(0, 10) === today
      );
  ...
  const { data: venuesData } = useSWR(
    activeTab === "venues" && seasonId
      ? `/api/seasons/${seasonId}/venues`
      : null,
    fetcher
  );
            ...
            <div className="p-6 space-y-8">
              ...
              <div>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-medium text-white line-clamp-2 mb-2">
                          {leagueData.name} updates and latest news will appear
                          here
                        </h3>
                ...
                <p className="text-sm text-sky-100/60 mt-4 text-center">
                  News integration coming soon. Connect your news API to show
                  real articles.
                </p>
              </div>
              ...
              {fixturesLoading ? (
                ...
                <div className="text-center py-12 bg-slate-900/50 border border-white/10 rounded-2xl backdrop-blur-sm">
                  ...
                  <p className="text-sm text-sky-100/70 mt-2">
                    Matches will appear here once the schedule is announced
                  </p>
                </div>
              ) : (
                <>
                  ...
                  {(() => {
                    ...
                    if (selectedDate) {
                      filteredData = sortedFixtures.filter(
                        (match: any) =>
                    ...
                    if (selectedDate && filteredData.length === 0) {
                      return (
                        <div className="text-center py-12 bg-slate-900/50 border border-white/10 rounded-2xl backdrop-blur-sm">
                          ...
                          <p className="text-sm text-sky-100/70 mt-2">
                            Try selecting a different date or view all matches
                          </p>
                          <button
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-white/10">
                        {standingsData.data?.map(
                          (standing: any, index: number) => {
                            const isTop3 = index < 3;
                            const isLast =
                              index === standingsData.data.length - 1;
                            return (
                              <tr
                                key={standing.id}
              ...
              {fixturesLoading ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                  <p className="text-gray-600">
                    No teams data available for this series
                  </p>
                </div>
              ) : (
          ...
          {activeTab === "venues" && (
            <div className="p-6">
              ...
              {!venuesData ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                  <p className="text-gray-600">
                    No venues data available for this series
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
