    ...
    <div className="space-y-3">
      {items.map((n) => (
        <Link
          key={n.id}
          href={`/news/${n.slug}`}
          className="group flex gap-4 rounded-2xl bg-white/5 p-3.5 ring-1 ring-white/10
        ...
        >
          ...
          <div className="relative h-[64px] flex-shrink-0 overflow-hidden rounded-2xl bg-black/40 ring-1 ring-white/10">
            <img
              src={n.imgSrc}
              alt={n.title}
              className="h-full w-full max-h-auto max-w-auto object-contain transition-transform duration-500 ease-out group-hover:scale-[1.03]"
              loading="lazy"
            />
          </div>
