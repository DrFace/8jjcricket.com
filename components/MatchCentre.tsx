...
export default function MatchCentre() {
    ...
    return (
        // ✅ critical: h-full + min-h-0 so children can scroll
        <div className="flex h-full min-h-0 w-full flex-col">
            {/* Header (fixed) */}
            <div className="mb-3 flex flex-shrink-0 flex-wrap items-start justify-between gap-3">
                <div>
            ...
            {/* Main card: takes remaining height, clips overflow */}
            <div className="flex-1 min-h-0 overflow-hidden rounded-2xl border border-white/15 bg-black/40 shadow-2xl backdrop-blur-2xl">
                {/* Tabs (fixed) */}
                <div className="flex flex-shrink-0 items-center gap-6 border-b border-white/10 bg-black/30 px-4 py-3">
                    <button className="border-b-2 border-amber-300 pb-2 text-xs font-semibold text-amber-200 sm:text-sm">
                        Live
                    </button>
                    <Link
                        href="/upcoming"
                        className="pb-2 text-xs font-semibold text-sky-100/70 hover:text-sky-100 sm:text-sm"
                    >
                        Upcoming
                    </Link>
                    <Link
                ...
                {/* ✅ Scroll area: must be flex-1 + min-h-0 + overflow-y-auto */}
