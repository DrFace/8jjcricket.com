...
export default function Scoreboard({ id }: { id: string }) {
  ...
  return (
    <div className="space-y-6">
      {/* League + header */}
      <div className="card">
        <h2 className="text-center text-xl font-semibold text-[#134486] mb-3">
          ...
          <div className="mt-2 rounded-md bg-gray-50 px-3 py-2 text-sm">
            ...
            {fx.elected ? (
              <>
                ...
                <span className="font-medium">{String(fx.elected).toLowerCase()}</span> first.
              </>
            ) : null}
          </div>
        )}
              ...
              return (
                <div key={inn.id ?? `${inn.team_id}-${inn.inning}`} className="border rounded-lg p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-xs text-gray-600">Inn {inn.inning ?? '—'}</div>
                  </div>
            ...
            <div key={tid} className="card space-y-4">
              ...
              <div>
                <h3 className="font-semibold mb-2">
                  {(teamObj?.short_name || teamObj?.name || team.label || `Team ${tid}`)} Batting
                </h3>
                <div className="overflow-x-auto">
                  <table className="min-w-[640px] w-full text-sm">
                    <thead className="text-left text-white bg-[#134486]">
                      <tr>
              ...
              <div>
                <h3 className="font-semibold mb-2">
                  {(teamObj?.short_name || teamObj?.name || team.label || `Team ${tid}`)} Bowling
                </h3>
                <div className="overflow-x-auto">
                  <table className="min-w-[640px] w-full text-sm">
                    <thead className="text-left text-white bg-[#134486]">
