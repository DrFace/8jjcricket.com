...
export default function Scoreboard({ id }: { id: string }) {
  ...
  const playerIds = useMemo(() => {
    const ids = [...batting.map((b) => Number(b.player_id)), ...bowling.map((b) => Number(b.player_id))]
  ...
  if (error) {
    return (
      <Panel>
        ...
        <div className="mt-1 text-xs text-white/60">{String((error as any).message)}</div>
      </Panel>
    )
  }
  if (isLoading) return <Panel className="animate-pulse text-white/70">Loading scoreboard…</Panel>
  ...
  const statusTone =
    statusText.toLowerCase().includes("finish") || statusText.toLowerCase().includes("completed")
      ? "success"
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-xl border border-white/10 bg-[#060A12] px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="text-xs font-semibold tracking-widest text-white/50">VS</div>
      ...
      {runs.length > 0 && (
        <Panel>
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-white">Innings Summary</h3>
      ...
      {(
        ...
        .map((team) => {
          ...
          const teamLabel = teamObj?.short_name || teamObj?.name || team.label || `Team ${tid}`
            ...
            <Panel key={tid} className="space-y-6">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-lg font-bold text-white">{teamLabel}</div>
                </div>
                <div className="flex items-center gap-2">
                  <Pill>Batting</Pill>
                  <Pill>Bowling</Pill>
              ...
              <div className="rounded-xl border border-white/10 bg-[#060A12] overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                  <h3 className="text-sm font-semibold text-white">{teamLabel} Batting</h3>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-[720px] w-full text-sm">
                    <thead className="bg-white/5 text-white/80">
                      <tr>
                        <th className="py-2 px-4 text-left font-semibold">Batter</th>
                    ...
                    <tbody className="divide-y divide-white/10">
                      {bats.length ? (
                        bats.map((b: AnyRow) => {
                          ...
                          return (
                            <tr key={b.id ?? `${tid}-bat-${pid}-${b.score}-${b.ball}`} className="hover:bg-white/5">
                              <td className="py-2 px-4">
                                {pid ? (
                                  <Link href={`/players/${pid}`} className="text-white hover:underline">
                                    {label}
                                  </Link>
                                ) : (
                              ...
                              <td className="py-2 px-3 text-center text-white/80">{b.score ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.ball ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.four_x ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.six_x ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.rate ?? "–"}</td>
                            </tr>
                          )
                        })
                      ) : (
                        Array.from({ length: 5 }).map((_, i) => (
                          <tr key={i}>
                            ...
                            <td className="py-2 px-3 text-center text-white/60">–</td>
                            <td className="py-2 px-3 text-center text-white/60">–</td>
                            <td className="py-2 px-3 text-center text-white/60">–</td>
                            <td className="py-2 px-3 text-center text-white/60">–</td>
                            <td className="py-2 px-3 text-center text-white/60">–</td>
              ...
              <div className="rounded-xl border border-white/10 bg-[#060A12] overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                  <h3 className="text-sm font-semibold text-white">{teamLabel} Bowling</h3>
                </div>
                    ...
                    <tbody className="divide-y divide-white/10">
                      {bowls.length ? (
                        bowls.map((b: AnyRow) => {
                          ...
                          const pid = b.player_id ? Number(b.player_id) : undefined
                          return (
                            <tr key={b.id ?? `${tid}-bowl-${pid}-${b.overs}-${b.runs}`} className="hover:bg-white/5">
                              <td className="py-2 px-4">
                                {pid ? (
                                  <Link href={`/players/${pid}`} className="text-white hover:underline">
                                    {label}
                              ...
                              <td className="py-2 px-3 text-center text-white/80">{b.overs ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.medians ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.runs ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.wickets ?? "–"}</td>
                              <td className="py-2 px-3 text-center text-white/80">{b.rate ?? "–"}</td>
                            </tr>
                          )
                        })
