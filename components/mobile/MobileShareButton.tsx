...
function ShareModal({
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center" role="dialog" aria-modal="true">
            ...
            {/* Bottom sheet look for mobile */}
            <div className="relative w-full sm:w-[520px] rounded-t-2xl sm:rounded-2xl border border-white/10 bg-slate-950/90 backdrop-blur-xl p-5 shadow-2xl">
                <div className="flex items-center justify-between mb-4">
                    <div className="text-base font-semibold text-slate-100">Share</div>
                ...
                <div className="grid grid-cols-4 gap-4">
                    ...
                    <button type="button" onClick={onCopy} className="group flex flex-col items-center gap-2 text-center">
                        ...
                        <span className="text-xs text-slate-200">{copied ? "Copied" : "Copy"}</span>
                    </button>
...
export default function MobileShareButton({
    ...
    const url = useMemo(() => {
        ...
        // IMPORTANT: mobile route
        return `${base}/mobile/news/${encodeURIComponent(slug)}`;
    }, [slug]);
    ...
    return (
        <>
            <button
                ...
                className={[
                    "inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-slate-200 hover:bg-white/10 transition",
                    className,
                ].join(" ")}
                aria-label="Share"
            >
