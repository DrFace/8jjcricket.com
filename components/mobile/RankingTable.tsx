...
export default function RankingTable({ data, title, onViewAll }: RankingTableProps) {
        ...
        <section className="p-[1px] rounded-2xl bg-gradient-to-b from-[#FFD100]/50 to-transparent">
            <div className="bg-[#0B0E14] rounded-2xl p-4 overflow-hidden">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-sm font-bold text-[#FFD100] uppercase tracking-wider">{title}</h2>
                    <span 
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full border-separate border-spacing-y-2">
                        <thead>
                            <tr>
                                {["Pos", "Team", "Matches", "Points", "Rating"].map((header) => (
                                    <th key={header} className="px-4 py-2 text-left text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                                        {header}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody>
                            {data.map((team) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-400">{team.ranking.matches}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-400">{team.ranking.points}</td>
